...
if __name__ == '__main__':
    ### define logger
    lg.basicConfig(filename='gaps.log', filemode='w', format='%(name)s - %(levelname)s - %(message)s')
    logger = lg.getLogger('gaps')
    ...
    try:
        dd.read_json(f'{args["out_path"]}/*/*.json', orient='records', lines=False)
    except:
        ...
        start_date = dd.read_json(f'{args["out_path"]}/*/*.json', orient='records', lines=False)
    ...
    for attempt in range(max_attemps+1):
        if attempt < max_attemps:
            ...
            try:
                ...
                for n, entry in enumerate(fun.scantree(args["input"])):
                    ### check starting date
